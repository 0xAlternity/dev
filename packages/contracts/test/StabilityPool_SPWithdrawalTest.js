...
contract('StabilityPool - Withdrawal of stability deposit - Reward calculations', async accounts => {
  ...
  describe("Stability Pool Withdrawal", async () => {
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      // Defaulter opens trove with 200% ICR and 21k LUSD net debt
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(21000, 18)), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(200, 'ether') })
      ...
      // Check depositors' compounded deposit is 3000 LUSD and ETH Gain is 66.3333 ETH
      ...
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(alice)).toString(), dec(3000, 18)), 10000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(bob)).toString(), dec(3000, 18)), 10000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(carol)).toString(), dec(3000, 18)), 10000)
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two identical liquidations", async () => {
      // Whale opens Trove with 200k ETH
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(200000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })

      // Whale transfers 20k LUSD to A, B and C who then deposit it to the SP
      const depositors = [alice, bob, carol]
      for (account of depositors) {
        await lusdToken.transfer(account, dec(20000, 18), { from: whale })
        await stabilityPool.provideToSP(dec(20000, 18), ZERO_ADDRESS, { from: account })
      }

      // Defaulters open trove with 200% ICR
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(21000, 18)), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(200, 'ether') })
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(21000, 18)), defaulter_2, defaulter_2, { from: defaulter_2, value: dec(200, 'ether') })
      ...
      // Check depositors' compounded deposit is 6000 LUSD and ETH Gain is 132.666 ETH
      const txA = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: alice })
      const txB = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: bob })
      const txC = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: carol })
      // Grab the ETH gain from the emitted event in the tx log
      ...
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(alice)).toString(), dec(6000, 18)), 20000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(bob)).toString(), dec(6000, 18)), 20000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(carol)).toString(), dec(6000, 18)), 20000)

      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, '132666666666666666666'), 20000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn,   '132666666666666666666'), 20000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, '132666666666666666666'), 20000)
    })
    ...
    it("withdrawFromSP():  Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three identical liquidations", async () => {
      ...
      // Whale transfers 30k LUSD to A, B and C who then deposit it to the SP
      const depositors = [alice, bob, carol]
      for (account of depositors) {
        await lusdToken.transfer(account, dec(20000, 18), { from: whale })
        await stabilityPool.provideToSP(dec(20000, 18), ZERO_ADDRESS, { from: account })
      ...
      // Check depositors' compounded deposit is 0 LUSD and ETH Gain is 198.999 ETH 
      const txA = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: alice })
      const txB = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: bob })
      const txC = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: carol })
      ...
      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, '198999999999999999999'), 10000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn,   '198999999999999999999'), 10000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, '198999999999999999999'), 10000)
    })
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two liquidations of increasing LUSD", async () => {
      ...
      // Defaulters open trove with 200% ICR
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(15000, 18)), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(150, 18) })
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(17000, 18)), defaulter_2, defaulter_2, { from: defaulter_2, value: dec(170, 18) })
      ...
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(alice)).toString(), '9333333333333333333333'), 20000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(bob)).toString(),   '9333333333333333333333'), 20000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(carol)).toString(), '9333333333333333333333'), 20000)

      // (150 + 170) * 0.995 / 3
      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, '106133333333333333333'), 20000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn,   '106133333333333333333'), 20000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, '106133333333333333333'), 20000)
    })
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three liquidations of increasing LUSD", async () => {
      ...
      // Defaulters open trove with 200% ICR
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(13000, 18)), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(130, 18) })
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(14000, 18)), defaulter_2, defaulter_2, { from: defaulter_2, value: dec(140, 18) })
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(15000, 18)), defaulter_3, defaulter_3, { from: defaulter_3, value: dec(150, 18) })
      ...
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(alice)).toString(), '6000000000000000000000'), 20000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(bob)).toString(), '6000000000000000000000'), 20000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(carol)).toString(), '6000000000000000000000'), 20000)

      // (130 + 140 + 150) * 0.995 / 3
      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, dec(1393, 17)), 10000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn,   dec(1393, 17)), 10000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, dec(1393, 17)), 10000)
    })
    ...
    it("withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after two identical liquidations", async () => {
      ...
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(15000, 18)), defaulter_2, defaulter_2, { from: defaulter_2, value: dec(150, 'ether') })
      ...
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(alice)).toString(), dec(5000, 18)), 100000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(bob)).toString(), dec(10000, 18)), 100000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(carol)).toString(), dec(15000, 18)), 100000)

      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, '49749999999999999999'), 100000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, '99499999999999999999'), 100000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, '149249999999999999999'), 100000)
    })
    ...
    it("withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three identical liquidations", async () => {
      ...
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(alice)).toString(), dec(2500, 18)), 100000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(bob)).toString(), dec(5000, 18)), 100000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(carol)).toString(), dec(7500, 18)), 100000)

      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, '74624999999999999999'), 100000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, '149249999999999999999'), 100000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, '223874999999999999999'), 100000)
    })
    ...
    it("withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three varying liquidations", async () => {
      ...
      Defaulter 2: 15000 LUSD & 50 ETH
      Defaulter 3: 46700 LUSD & 500 ETH
      */
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(200_000, 18)), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(2000, 'ether') })
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(15_000, 18)), defaulter_2, defaulter_2, { from: defaulter_2, value: dec(150, 'ether') })
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(45_000, 18)), defaulter_3, defaulter_3, { from: defaulter_3, value: dec(450, 'ether') })
      ...
      // ()
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(alice)).toString(), '896200382084483122000'), 100000000000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(bob)).toString(), '204333687115262151816000'), 10000000000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(carol)).toString(), '5870112502653364449100'), 10000000000)

      // 2710 * 0.995 * {2000, 456000, 13100}/4711
      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, '10982806198259392589'), 10000000000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, '2504079813203141510395'), 10000000000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, '71937380598599021460'), 10000000000)
    })
    ...
    it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 LUSD.  A, B, C, D withdraw correct LUSD deposit and ETH Gain", async () => {
      ...
      // Whale transfers 15k LUSD to A, B and C who then deposit it to the SP
      const depositors = [alice, bob, carol]
      for (account of depositors) {
        await lusdToken.transfer(account, dec(15000, 18), { from: whale })
        await stabilityPool.provideToSP(dec(15000, 18), ZERO_ADDRESS, { from: account })
      }
      ...
      await lusdToken.transfer(dennis, dec(15000, 18), { from: whale })
      await stabilityPool.provideToSP(dec(15000, 18), ZERO_ADDRESS, { from: dennis })
      ...
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(alice)).toString(), dec(2500, 18)), 100000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(bob)).toString(), dec(2500, 18)), 100000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(carol)).toString(), dec(2500, 18)), 100000)

      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(dennis)).toString(), dec(7500, 18)), 100000)

      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, '124374999999999999999'), 100000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, '124374999999999999999'), 100000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, '124374999999999999999'), 100000)

      assert.isAtMost(th.getDifference(dennis_ETHWithdrawn, '74625000000000000000'), 100000)
    })
    ...
    it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 LUSD.  A, B, C, D withdraw correct LUSD deposit and ETH Gain", async () => {
      ...
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(15000, 18)), defaulter_2, defaulter_2, { from: defaulter_2, value: dec(150, 'ether') })
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(15000, 18)), defaulter_3, defaulter_3, { from: defaulter_3, value: dec(150, 'ether') })
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(15000, 18)), defaulter_4, defaulter_4, { from: defaulter_4, value: dec(150, 'ether') })
      ...
      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, dec(14925, 16)), 100000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, dec(14925, 16)), 100000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, dec(14925, 16)), 100000)
      assert.isAtMost(th.getDifference(dennis_ETHWithdrawn, dec(14925, 16)), 100000)
    })
    ...
    it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct LUSD deposit and ETH Gain", async () => {
      ...
      */
      // Whale transfers LUSD to  A, B and C respectively who then deposit it to the SP
      await lusdToken.transfer(alice, dec(70000, 18), { from: whale })
      await stabilityPool.provideToSP(dec(70000, 18), ZERO_ADDRESS, { from: alice })
      await lusdToken.transfer(bob, dec(30000, 18), { from: whale })
      await stabilityPool.provideToSP(dec(30000, 18), ZERO_ADDRESS, { from: bob })
      ...
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(alice)).toString(), '19687499999999999930000'), 100000000000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(bob)).toString(), '8437499999999999970000'), 100000000000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(carol)).toString(), '5624999999999999980000'), 100000000000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(dennis)).toString(), '11249999999999999977999'), 100000000000)

      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, '500609374999999991488'), 100000000000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, '214546874999999996352'), 100000000000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, '143031249999999997568'), 100000000000)
      assert.isAtMost(th.getDifference(dennis_ETHWithdrawn, '136812500000000008721'), 100000000000)
    })
    ...
    it("withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 LUSD.  A, B, C, D withdraw correct LUSD deposit and ETH Gain", async () => {
      ...
      const txD = await stabilityPool.withdrawFromSP(dec(15000, 18), { from: dennis })
      await priceFeed.setPrice(dec(100, 18))

      const dennis_ETHWithdrawn = th.getEventArgByName(txD, 'ETHGainWithdrawn', '_ETH').toString()
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(dennis)).toString(), dec(7500, 18)), 100000)
      assert.isAtMost(th.getDifference(dennis_ETHWithdrawn, '74624999999999992500'), 100000)
    ...
    it("withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct LUSD deposit and ETH Gain", async () => {
      ...
      */
      ...
      await lusdToken.transfer(alice, dec(25000, 18), { from: whale })
      await stabilityPool.provideToSP(dec(25000, 18), ZERO_ADDRESS, { from: alice })
      await lusdToken.transfer(bob, dec(25000, 18), { from: whale })
      await stabilityPool.provideToSP(dec(25000, 18), ZERO_ADDRESS, { from: bob })
      await lusdToken.transfer(carol, dec(25000, 18), { from: whale })
      await stabilityPool.provideToSP(dec(25000, 18), ZERO_ADDRESS, { from: carol })
      await lusdToken.transfer(dennis, dec(50000, 18), { from: whale })
      await stabilityPool.provideToSP(dec(50000, 18), ZERO_ADDRESS, { from: dennis })

      /* Defaulters open troves:
      Defaulter 1: 15000 LUSD
      Defaulter 2: 20000 LUSD
      Defaulter 3: 30000 LUSD
      Defaulter 4: 15000 LUSD
      */
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(15000, 18)), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(150, 'ether') })
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(20000, 18)), defaulter_2, defaulter_2, { from: defaulter_2, value: dec(200, 'ether') })
      ...
      // Increasing the price for a moment to avoid pending liquidations to block withdrawal
      await priceFeed.setPrice(dec(200, 18))
      const txD = await stabilityPool.withdrawFromSP(dec(50000, 18), { from: dennis })
      await priceFeed.setPrice(dec(100, 18))

      const dennis_ETHWithdrawn = th.getEventArgByName(txD, 'ETHGainWithdrawn', '_ETH').toString()
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(dennis)).toString(), dec(36000, 18)), 100000000000)
      // 350*0.995 * 50000/125000
      assert.isAtMost(th.getDifference(dennis_ETHWithdrawn, dec(1393, 17)), 100000000000)
      ...
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(alice)).toString(), dec(3000, 18)), 10000000000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(bob)).toString(), dec(3000, 18)), 100000000000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(carol)).toString(), dec(3000, 18)), 100000000000)

      // 350*0.995 * {20000,25000,12500}/97500 + 450*0.995 * {20000,25000,12500}/57500
      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, dec(2189, 17)), 100000000000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, dec(2189, 17)), 100000000000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, dec(2189, 17)), 100000000000)
    })
    ...
    it("withdrawFromSP(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 LUSD. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct LUSD deposit and ETH Gain", async () => {
      ...
      // Whale transfers 20k LUSD to A, B and D who then deposit it to the SP
      ...
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(15000, 18)), defaulter_2, defaulter_2, { from: defaulter_2, value: dec(150, 'ether') })
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(15000, 18)), defaulter_3, defaulter_3, { from: defaulter_3, value: dec(150, 'ether') })
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(13000, 18)), defaulter_4, defaulter_4, { from: defaulter_4, value: dec(130, 'ether') })
      ...
      const dennis_ETHWithdrawn = th.getEventArgByName(txD, 'ETHGainWithdrawn', '_ETH').toString()
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(dennis)).toString(), dec(7000, 18)), 100000)
      assert.isAtMost(th.getDifference(dennis_ETHWithdrawn, dec(12935, 16)), 100000)
      ...
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(alice)).toString(), dec(3750, 18)), 100000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(bob)).toString(), dec(3750, 18)), 100000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(carol)).toString(), dec(7500, 18)), 100000)

      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, dec(1616875, 14)), 100000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, dec(1616875, 14)), 100000)
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, dec(124375, 15)), 100000)
    })
    ...
    it("withdrawFromSP(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
      ...
      // Defaulter 2 liquidated. 5000 LUSD offset
      await troveManager.liquidate(defaulter_2, { from: owner });
      ...
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(carol)).toString(), dec(2500, 18)), 100000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(dennis)).toString(), dec(2500, 18)), 100000)

      // Expect Carol and and Dennis ETH Gain to be 50 ETH
      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, '74625000000000000000'), 100000)
      assert.isAtMost(th.getDifference(dennis_ETHWithdrawn, '74625000000000000000'), 100000)
    })
    ...
    it("withdrawFromSP(): Pool-emptying liquidation increases epoch by one, resets scaleFactor to 0, and resets P to 1e18", async () => {
      ...
      // Whale transfers 15k LUSD to A, B who then deposit it to the SP
      const depositors = [alice, bob]
      for (account of depositors) {
        await lusdToken.transfer(account, dec(15000, 18), { from: whale })
        await stabilityPool.provideToSP(dec(15000, 18), ZERO_ADDRESS, { from: account })
      ...
      // Carol, Dennis each deposit 15000 LUSD
      const depositors_2 = [carol, dennis]
      for (account of depositors) {
        await lusdToken.transfer(account, dec(15000, 18), { from: whale })
        await stabilityPool.provideToSP(dec(15000, 18), ZERO_ADDRESS, { from: account })
      ...
      // Defaulter 3 liquidated. 15000 LUSD fully offset, Pool remains non-zero
      await troveManager.liquidate(defaulter_3, { from: owner });
      ...
      // Defaulter 4 liquidated. 15000 LUSD, empties pool
      await troveManager.liquidate(defaulter_4, { from: owner });
    ...
    it("withdrawFromSP(): Depositors withdraw correct compounded deposit after liquidation empties the pool", async () => {
      ...
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(carol)).toString(), dec(7500, 18)), 100000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(dennis)).toString(), dec(15000, 18)), 100000)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(erin)).toString(), dec(22500, 18)), 100000)

      //Expect Alice and Bob's ETH Gain to be 1 ETH
      assert.isAtMost(th.getDifference(alice_ETHWithdrawn, dec(995, 17)), 100000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, dec(995, 17)), 100000)

      assert.isAtMost(th.getDifference(carol_ETHWithdrawn, dec(24875, 15)), 100000)
      assert.isAtMost(th.getDifference(dennis_ETHWithdrawn, dec(4975, 16)), 100000)
      assert.isAtMost(th.getDifference(erin_ETHWithdrawn, dec(74625, 15)), 100000)
    })
    ...
    it("withdrawFromSP(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      // Defaulter 1 liquidated.  Value of P reduced to 6e9.
      await troveManager.liquidate(defaulter_1, { from: owner });
      ...
      const txB = await stabilityPool.withdrawFromSP(dec(15000, 18), { from: bob })
      const bob_ETHWithdrawn = await th.getEventArgByName(txB, 'ETHGainWithdrawn', '_ETH').toString()

      // Expect Bob to withdraw 1% of initial deposit (100 LUSD) and all the liquidated ETH (140 ether)
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(bob)).toString(), '100000000000000000000'), 100000)
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, '139300000000000000000'), 100000)
    })
    ...
    it("withdrawFromSP(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      // Whale opens Trove with 100k ETH
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(1000000, 18)), whale, whale, { from: whale, value: dec(100000, 'ether') })
      ...
      // Defaulter 1 and default 2 each withdraw 99999.999999999 LUSD
      ...
      assert.isAtMost(th.getDifference((await lusdToken.balanceOf(bob)).toString(), dec(1, 18)), 100000)
    ...
    it("withdrawFromSP(): Several deposits of varying amounts span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      // Defaulter 1 and default 2 withdraw up to debt of 99999 LUSD and 59999.4 LUSD
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(99999, 18)), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(1000, 'ether') })
    ...
    */
    ...
    it("withdrawFromSP(): Depositor's ETH gain stops increasing after two scale changes", async () => {
      ...
      // Defaulters 1-5 each withdraw up to debt of 99999.9999999 LUSD
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(99999, 18)), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(1000, 'ether') })
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(99999, 18)), defaulter_2, defaulter_2, { from: defaulter_2, value: dec(1000, 'ether') })
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(99999, 18)), defaulter_3, defaulter_3, { from: defaulter_3, value: dec(1000, 'ether') })
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(99999, 18)), defaulter_4, defaulter_4, { from: defaulter_4, value: dec(1000, 'ether') })
