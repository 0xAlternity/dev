...
- [Alternity System Summary](#alternity-system-summary)
  ...
  - [LCNY Token Redemption](#lcny-token-redemption)
  ...
  - [Project Structure](#project-structure)
    ...
    - [Branches](#branches)
  - [ALTR Token Architecture](#altr-token-architecture)
    - [ALTR Lockup contracts and token vesting](#altr-lockup-contracts-and-token-vesting)
    ...
    - [Launch sequence and vesting process](#launch-sequence-and-vesting-process)
      - [Deploy ALTR Contracts](#deploy-altr-contracts)
      - [Deploy and fund Lockup Contracts](#deploy-and-fund-lockup-contracts)
      - [Deploy Alternity Core](#deploy-alternity-core)
  ...
  - [Core System Architecture](#core-system-architecture)
    ...
    - [Flow of Ether in Alternity](#flow-of-ether-in-alternity)
    - [Flow of LCNY tokens in Alternity](#flow-of-lcny-tokens-in-alternity)
    - [Flow of ALTR Tokens in Alternity](#flow-of-altr-tokens-in-alternity)
  ...
  - [Public User-Facing Functions](#public-user-facing-functions)
    ...
    - [ALTR Staking Functions  `LQTYStaking.sol`](#altr-staking-functions--lqtystakingsol)
    ...
    - [LCNY token `LUSDToken.sol` and ALTR token `LQTYToken.sol`](#lusd-token-lusdtokensol-and-altr-token-lqtytokensol)
  ...
  - [ALTR Issuance to Stability Providers](#altr-issuance-to-stability-providers)
    - [ALTR Issuance schedule](#altr-issuance-schedule)
    - [ALTR Issuance implementation](#altr-issuance-implementation)
    - [Handling the front end ALTR gain](#handling-the-front-end-altr-gain)
    - [ALTR reward events and payouts](#altr-reward-events-and-payouts)
  - [Alternity System Fees](#alternity-system-fees)
    ...
    - [Staking ALTR and earning fees](#staking-altr-and-earning-fees)
...
Alternity is a collateralized debt platform. Users can lock up Ether, and issue stablecoin tokens (LCNY) to their own Ethereum address, and subsequently transfer those tokens to any other Ethereum address. The individual collateralized debt positions are called Troves.

The stablecoin tokens are economically geared towards maintaining value of 1 LCNY = ¥1 CNY, due to the following properties:

1. The system is designed to always be over-collateralized - the ¥ value of the locked Ether exceeds the ¥ value of the issued stablecoins

2. The stablecoins are fully redeemable - users can always swap ¥ worth of LCNY for ¥ worth of ETH (minus fees), directly with the system.

3. The system algorithmically controls the generation of LCNY through a variable issuance fee.

After opening a Trove with some Ether, users may issue ("borrow") tokens such that the collateralization ratio of their Trove remains above 110%. A user with ¥10000 worth of ETH in a Trove can issue up to 9090 LCNY.
...
Alternity utilizes a two-step liquidation mechanism in the following order of priority: 

1. Offset under-collateralized Troves against the Stability Pool containing LCNY tokens

2. Redistribute under-collateralized Troves to other borrowers if the Stability Pool is emptied

Alternity primarily uses the LCNY tokens in its Stability Pool to absorb the under-collateralized debt, i.e. to repay the liquidated borrower's liability.

Any user may deposit LCNY tokens to the Stability Pool. This allows them to earn the collateral from the liquidated Trove. When a liquidation occurs, the liquidated debt is cancelled with the same amount of LCNY in the Pool (which is burned as a result), and the liquidated Ether is proportionally distributed to depositors.
...
If the liquidated debt is higher than the amount of LCNY in the Stability Pool, the system tries to cancel as much debt as possible with the tokens in the Stability Pool, and then redistributes the remaining liquidated collateral and debt across all active Troves.
...
Here is the liquidation logic for a single Trove in Normal Mode and Recovery Mode.  `SP.LCNY` represents the LCNY in the Stability Pool.

#### Liquidations in Normal Mode: TCR >= 150%

| &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Condition                      | Liquidation behavior                                                                                                                                                                                                                                                                                                |
|----------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ICR < MCR & SP.LUSD >= trove.debt | LCNY in the StabilityPool equal to the Trove's debt is offset with the Trove's debt. The Trove's ETH collateral is shared between depositors.                                                                                                                                                                       |
| ICR < MCR & SP.LUSD < trove.debt | The total StabilityPool LCNY is offset with an equal amount of debt from the Trove.  A fraction of the Trove's collateral (equal to the ratio of its offset debt to its entire debt) is shared between depositors. The remaining debt and collateral (minus ETH gas compensation) is redistributed to active Troves |
...
| 100% < ICR < MCR & SP.LUSD > trove.debt  | LCNY in the StabilityPool equal to the Trove's debt is offset with the Trove's debt. The Trove's ETH collateral (minus ETH gas compensation) is shared between depsitors.                                                                                                                                                                                                                                    |
| 100% < ICR < MCR & SP.LUSD < trove.debt  | The total StabilityPool LCNY is offset with an equal amount of debt from the Trove.  A fraction of the Trove's collateral (equal to the ratio of its offset debt to its entire debt) is shared between depositors. The remaining debt and collateral (minus ETH gas compensation) is redistributed to active troves                                                                                          |
| MCR <= ICR < TCR & SP.LUSD >= trove.debt  |  The Pool LCNY is offset with an equal amount of debt from the Trove. A fraction of ETH collateral with dollar value equal to `1.1 * debt` is shared between depositors. Nothing is redistributed to other active Troves. Since it's ICR was > 1.1, the Trove has a collateral remainder, which is sent to the `CollSurplusPool` and is claimable by the borrower. The Trove is closed. |
...
Similarly, a Trove's accumulated gains from liquidations are automatically applied to the Trove when the owner performs any operation - e.g. adding/withdrawing collateral, or issuing/repaying LCNY.

## LCNY Token Redemption

Any LCNY holder (whether or not they have an active Trove) may redeem their LCNY directly with the system. Their LCNY is exchanged for ETH, at face value: redeeming x LCNY tokens returns ¥ worth of ETH (minus a [redemption fee](#redemption-fee)).

When LCNY is redeemed for ETH, the system cancels the LCNY with debt from Troves, and the ETH is drawn from their collateral.
...
Redemptions are blocked when TCR < 110% (there is no need to restrict ICR < TCR). At that TCR redemptions would likely be unprofitable, as LCNY is probably trading above ¥1 if the system has crashed that badly, but it could be a way for an attacker with a lot of LCNY to lower the TCR even further.

Note that redemptions are disabled during the first 14 days of operation since deployment of the Alternity protocol to protect the monetary system in its infancy.
...
A Trove is defined as “fully redeemed from” when the redemption has caused (debt-50) of its debt to absorb (debt-50) LCNY. Then, its 2000 LCNY Liquidation Reserve is cancelled with its remaining 2000 debt: the Liquidation Reserve is burned from the gas address, and the 2000 debt is zero’d.
...
Economically, the redemption mechanism creates a hard price floor for LCNY, ensuring that the market price stays at or near to ¥1 LCNY. 

## Recovery Mode

Recovery Mode kicks in when the total collateralization ratio (TCR) of the system falls below 150%.

During Recovery Mode, liquidation conditions are relaxed, and the system blocks borrower transactions that would further decrease the TCR. New LCNY may only be issued by adjusting existing Troves in a way that improves their ICR, or by opening a new Trove with an ICR of >=150%. In general, if an existing Trove's adjustment reduces its ICR, the transaction is only executed if the resulting TCR is above 150%

Recovery Mode is structured to incentivize borrowers to behave in ways that promptly raise the TCR back above 150%, and to incentivize LCNY holders to replenish the Stability Pool.
...
- `packages/dev-frontend/` - Developer UI: a fully functional React app used for interfacing with the smart contracts during development
...
- `packages/lib-ethers/` - [Ethers](https://github.com/ethers-io/ethers.js/)-based middleware that can read state and send transactions
- `packages/lib-react/` - Components and hooks that React-based apps can use to view contract state
- `packages/lib-subgraph/` - [Apollo Client](https://github.com/apollographql/apollo-client)-based middleware backed by the subgraph that can read state
- `packages/providers/` - Subclassed Ethers providers used by the frontend
- `packages/subgraph/` - [Subgraph](https://thegraph.com) for querying state as well as historical data like transaction history
...
Backend development is done in the Hardhat framework, and allows Alternity to be deployed on the Hardhat EVM network for fast compilation and test execution.

### Branches

As of 18/09/2023, the current working branch is `main`. `master` is out of date.

## ALTR Token Architecture

The Alternity system incorporates a secondary token, ALTR. This token entitles the holder to a share of the system revenue generated by redemption fees and  issuance fees.

To earn a share of system fees, the ALTR holder must stake their ALTR in a staking contract.

Alternity also issues ALTR to Stability Providers, in a continous time-based manner.

The ALTR contracts consist of:

`LQTYStaking.sol` - the staking contract, containing stake and unstake functionality for ALTR holders. This contract receives ETH fees from redemptions, and LCNY fees from new debt issuance.

`CommunityIssuance.sol` - This contract handles the issuance of ALTR tokens to Stability Providers as a function of time. It is controlled by the `StabilityPool`. Upon system launch, the `CommunityIssuance` automatically receives 50 million ALTR - the “community issuance” supply. The contract steadily issues these ALTR tokens to the Stability Providers over time.

`LQTYToken.sol` - This is the ALTR ERC20 contract. It has a hard cap supply of 100 million. 
**Note that the admin address has no extra privileges and does not retain any control over the protocol once deployed.**

### ALTR Lockup contracts and token vesting

Some ALTR is reserved for team members and partners, and is locked up for three year upon system launch. Additionally, some team members receive ALTR vested on a monthly basis, which during the first year, is transferred directly to their lockup contract.

Also, separate ALTR allocations are made at deployent to an EOA that will hold an amount of ALTR for bug bounties/hackathons and to the airdrop contract. Aside from these allocations, the only ALTR made freely available in this first year is the ALTR that is publically issued to Stability Providers via the `CommunityIssuance` contract.

### Launch sequence and vesting process

#### Deploy ALTR Contracts
1. Deployer deploys `LockupContractFactory`
2. Deployer deploys `CommunityIssuance`
3. Deployer deploys `LQTYStaking` 
4. Deployer deploys `LQTYToken`, which upon deployment:
- Stores the `CommunityIssuance` and `LockupContractFactory` addresses
- Mints ALTR tokens to `CommunityIssuance`, the deployer address, the `Airdrop` contract address, and the bug bounty address
5. Deployer sets `LQTYToken` address in `LockupContractFactory`, `CommunityIssuance`, `LQTYStaking`, and `Airdrop`

#### Deploy and fund Lockup Contracts
6. Deployer tells `LockupContractFactory` to deploy a `LockupContract` for each beneficiary, with an `unlockTime` set to exactly schedule from documentation
7. Deployer transfers ALTR to each `LockupContract`, according to their entitlement

#### Deploy Alternity Core
8. Deployer deploys the Alternity core system
9. Deployer connects Alternity core system internally (with setters)
10. Deployer connects `LQTYStaking` to Alternity core contracts and `LQTYToken`
11. Deployer connects `CommunityIssuance` to Alternity core contracts and `LQTYToken`

_NOTE: In the final architecture, a multi-sig contract will be used to move ALTR Tokens, rather than the EOA. It will be deployed at the start of the sequence, and have its address recorded in  `LQTYToken` in step 4, and receive ALTR tokens. It will be used to move ALTR in step 7, and during & after the lockup period. The EOA will only be used for deployment of contracts in steps 1-4 and 9._

_The current code does not utilize a multi-sig. It implements the launch architecture outlined above._

## Core System Architecture

The core Alternity system consists of several smart contracts, which are deployable to the Ethereum blockchain.

All application logic and data is contained in these contracts - there is no need for a separate database or back end logic running on a web server. In effect, the Ethereum network is itself the Alternity back end. As such, all balances and contract data are public.
...
The three main contracts - `BorrowerOperations.sol`, `TroveManager.sol` and `StabilityPool.sol` - hold the user-facing public functions, and contain most of the internal system logic. Together they control Trove state updates and movements of Ether and LCNY tokens around the system.
...
`StabilityPool.sol` - contains functionality for Stability Pool operations: making deposits, and withdrawing compounded deposits and accumulated ETH and ALTR gains. Holds the LCNY Stability Pool deposits, and the ETH gains for depositors, from liquidations.

`LUSDToken.sol` - the stablecoin token contract, which implements the ERC20 fungible token standard in conjunction with EIP-2612 and a mechanism that blocks (accidental) transfers to addresses like the StabilityPool and address(0) that are not supposed to receive funds through direct transfers. The contract mints, burns and transfers LCNY tokens.
...
`PriceFeed.sol` - Contains functionality for obtaining the current ETH:CNY price, which the system uses for calculating collateralization ratios.
...
`GasPool.sol` - holds the total LCNY liquidation reserves. LCNY is moved into the `GasPool` when a Trove is opened, and moved out when a Trove is liquidated or closed.
...
Alternity functions that require the most current ETH:CNY price data fetch the price dynamically, as needed, via the core `PriceFeed.sol` contract using the Chainlink ETH:USD and CNY:USD reference contract as its primary and Tellor's ETH:USD and CNY:USD price feed as its secondary (fallback) data source. PriceFeed is stateful, i.e. it records the last good price that may come from either of the two sources based on the contract's current state.

The fallback logic distinguishes 3 different failure modes for Chainlink and 2 failure modes for Tellor:

- `Frozen` (for both oracles): last price update more than 32 hours ago
...
The current `PriceFeed.sol` contract has an external `fetchPrice()` function that is called by core Alternity functions which require a current ETH:CNY price.  `fetchPrice()` calls each oracle's proxy, asserts on the responses, and converts returned prices to 18 digits.
...
Alternity relies on a particular data structure: a sorted doubly-linked list of Troves that remains ordered by individual collateralization ratio (ICR), i.e. the amount of collateral (in CNY) divided by the amount of debt (in LCNY).
...
- The ETH:CNY price varies, altering the CNY of the collateral of every Trove
...
Nodes also remain sorted as the ETH:CNY price varies, since price fluctuations change the collateral value of each Trove by the same proportion.
...
### Flow of LCNY tokens in Alternity

![Flow of LUSD](images/LUSD_flows.svg)

When a user issues debt from their Trove, LCNY tokens are minted to their own address, and a debt is recorded on the Trove. Conversely, when they repay their Trove’s LCNY debt, LCNY is burned from their address, and the debt on their Trove is reduced.

Redemptions burn LCNY from the redeemer’s balance, and reduce the debt of the Trove redeemed against.

Liquidations that involve a Stability Pool offset burn tokens from the Stability Pool’s balance, and reduce the LCNY debt of the liquidated Trove.

The only time LCNY is transferred to/from a Alternity contract, is when a user deposits LCNY to, or withdraws LCNY from, the StabilityPool.

**Borrower Operations**

| Function                      | LCNY Quantity | ERC20 Operation                      |
|-------------------------------|---------------|--------------------------------------|
| openTrove                     | Drawn LCNY    | LUSD._mint(msg.sender, _LUSDAmount)  |
|                               | Issuance fee  | LUSD._mint(LQTYStaking,  LUSDFee)    |
| withdrawLUSD                  | Drawn LCNY    | LUSD._mint(msg.sender, _LUSDAmount)  |
|                               | Issuance fee  | LUSD._mint(LQTYStaking,  LUSDFee)    |
| repayLUSD                     | Repaid LCNY   | LUSD._burn(msg.sender, _LUSDAmount)  |
| adjustTrove: withdrawing LUSD | Drawn LCNY    | LUSD._mint(msg.sender, _LUSDAmount)  |
|                               | Issuance fee  | LUSD._mint(LQTYStaking,  LUSDFee)    |
| adjustTrove: repaying LUSD    | Repaid LCNY   | LUSD._burn(msg.sender, _LUSDAmount)  |
| closeTrove                    | Repaid LCNY   | LUSD._burn(msg.sender, _LUSDAmount) |

**Trove Manager**

| Function                 | LCNY Quantity            | ERC20 Operation                                  |
|--------------------------|--------------------------|--------------------------------------------------|
| liquidate (offset)       | LCNY to offset with debt | LUSD._burn(stabilityPoolAddress, _debtToOffset); |
| liquidateTroves (offset)   | LCNY to offset with debt | LUSD._burn(stabilityPoolAddress, _debtToOffset); |
| batchLiquidateTroves (offset) | LCNY to offset with debt | LUSD._burn(stabilityPoolAddress, _debtToOffset); |
| redeemCollateral         | LCNY to redeem           | LUSD._burn(msg.sender, _LUSD)                    |
...
| Function | LCNY Quantity                                   | ERC20 Operation                                           |
|----------|-------------------------------------------------|-----------------------------------------------------------|
| stake    | staker's accumulated LCNY gain from system fees | LUSD._transfer(LQTYStakingAddress, msg.sender, LUSDGain); |
| unstake  | staker's accumulated LCNY gain from system fees | LUSD._transfer(LQTYStakingAddress, msg.sender, LUSDGain); |

### Flow of ALTR Tokens in Alternity

![Flow of ALTR](images/LQTY_flows.svg)

Stability Providers and Frontend Operators receive ALTR gains according to their share of the total LCNY deposits, and the ALTR community issuance schedule.  Once obtained, ALTR can be staked and unstaked with the `LQTYStaking` contract.

**Stability Pool**

| Function               | ALTR Quantity       | ERC20 Operation                                                       |
|------------------------|---------------------|-----------------------------------------------------------------------|
| provideToSP            | depositor ALTR gain | LQTY._transfer(stabilityPoolAddress, msg.sender, depositorLQTYGain); |
|                        | front end ALTR gain | LQTY._transfer(stabilityPoolAddress, _frontEnd, frontEndLQTYGain);   |
| withdrawFromSP         | depositor ALTR gain | LQTY._transfer(stabilityPoolAddress, msg.sender, depositorLQTYGain); |
|                        | front end ALTR gain | LQTY._transfer(stabilityPoolAddress, _frontEnd, frontEndLQTYGain);   |
| withdrawETHGainToTrove | depositor ALTR gain | LQTY._transfer(stabilityPoolAddress, msg.sender, depositorLQTYGain); |
|                        | front end ALTR gain | LQTY._transfer(stabilityPoolAddress, _frontEnd, frontEndLQTYGain);   |

**ALTR Staking Contract**

| Function | ALTR Quantity                  | ERC20 Operation                                           |
|----------|--------------------------------|-----------------------------------------------------------|
| stake    | staker's ALTR deposit / top-up | LQTY._transfer(msg.sender, LQTYStakingAddress, _amount); |
| unstake  | staker's ALTR withdrawal       | LQTY._transfer(LQTYStakingAddress, msg.sender, _amount); |
...
ALTR token holders may also redeem their tokens, and swap an amount of tokens 1-for-1 in value (minus fees) with Ether.

ALTR token holders may stake their ALTR, to earn a share of the system fee revenue, in ETH and LCNY.

## Contract Ownership and Function Permissions

All the core smart contracts inherit from the OpenZeppelin `Ownable.sol` contract template. As such all contracts have a single owning address, which is the deploying address. The contract's ownership is renounced either upon deployment, or immediately after its address setter has been called, connecting it to the rest of the core Alternity system. 
...
`function withdrawLUSD(uint _maxFeePercentage, uint _LUSDAmount, address _upperHint, address _lowerHint)`: issues `_amount` of LCNY from the caller’s Trove to the caller. Executes only if the Trove's collateralization ratio would remain above the minimum, and the resulting total collateralization ratio is above 150%. The borrower has to provide a `_maxFeePercentage` that he/she is willing to accept in case of a fee slippage, i.e. when a redemption transaction is processed first, driving up the issuance fee.

`repayLUSD(uint _amount, address _upperHint, address _lowerHint)`: repay `_amount` of LCNY to the caller’s Trove, subject to leaving 2000 debt in the Trove (which corresponds to the 2000 LCNY gas compensation).
...
`closeTrove()`: allows a borrower to repay all debt, withdraw all their collateral, and close their Trove. Requires the borrower have a L balance sufficient to repay their trove's debt, excluding gas compensation - i.e. `(debt - 0)` LCN.
...
`redeemCollateral(uint _LUSDAmount, address _firstRedemptionHint, address _upperPartialRedemptionHint, address _lowerPartialRedemptionHint, uint _partialRedemptionHintNICR, uint _maxIterations, uint _maxFeePercentage)`: redeems `_LUSDamount` of stablecoins for ether from the system. Decreases the caller’s LCNY balance, and sends them the corresponding amount of ETH. Executes successfully if the caller has sufficient LCNY to redeem. The number of Troves redeemed from is capped by `_maxIterations`. The borrower has to provide a `_maxFeePercentage` that he/she is willing to accept in case of a fee slippage, i.e. when another redemption transaction is processed first, driving up the redemption fee.

`getCurrentICR(address _user, uint _price)`: computes the user’s individual collateralization ratio (ICR) based on their total collateral and total LCNY debt. Returns 2^256 -1 if they have 0 debt.
...
`getEntireSystemDebt()` Returns the systemic entire debt assigned to Troves, i.e. the sum of the LCNY debt in the Active Pool and the Default Pool.
...
`provideToSP(uint _amount, address _frontEndTag)`: allows stablecoin holders to deposit `_amount` of LCNY to the Stability Pool. It sends `_amount` of LCNY from their address to the Pool, and tops up their LCNY deposit by `_amount` and their tagged front end’s stake by `_amount`. If the depositor already has a non-zero deposit, it sends their accumulated ETH and ALTR gains to their address, and pays out their front end’s ALTR gain to their front end.

`withdrawFromSP(uint _amount)`: allows a stablecoin holder to withdraw `_amount` of LCNY from the Stability Pool, up to the value of their remaining Stability deposit. It decreases their LCNY balance by `_amount` and decreases their front end’s stake by `_amount`. It sends the depositor’s accumulated ETH and ALTR gains to their address, and pays out their front end’s ALTR gain to their front end. If the user makes a partial withdrawal, their deposit remainder will earn further gains. To prevent potential loss evasion by depositors, withdrawals from the Stability Pool are suspended when there are liquidable Troves with ICR < 110% in the system.

`withdrawETHGainToTrove(address _hint)`: sends the user's entire accumulated ETH gain to the user's active Trove, and updates their Stability deposit with its accumulated loss from debt absorptions. Sends the depositor's ALTR gain to the depositor, and sends the tagged front end's ALTR gain to the front end.
...
`getDepositorLQTYGain(address _depositor)`: returns the accumulated ALTR gain for a given Stability Pool depositor

`getFrontEndLQTYGain(address _frontEnd)`: returns the accumulated ALTR gain for a given front end
...
### ALTR Staking Functions  `LQTYStaking.sol`

 `stake(uint _LQTYamount)`: sends `_LQTYAmount` from the caller to the staking contract, and increases their stake. If the caller already has a non-zero stake, it pays out their accumulated ETH and LCNY gains from staking.

 `unstake(uint _LQTYamount)`: reduces the caller’s stake by `_LQTYamount`, up to a maximum of their entire stake. It pays out their accumulated ETH and LCNY gains from staking.

### Lockup Contract Factory `LockupContractFactory.sol`

`deployLockupContract(address _beneficiary, uint _unlockTime)`; Deploys a `LockupContract`, and sets the beneficiary’s address, and the `_unlockTime` - the instant in time at which the ALTR can be withrawn by the beneficiary.

### Lockup contract - `LockupContract.sol`

`withdrawLQTY()`: When the current time is later than the `unlockTime` and the caller is the beneficiary, it transfers their ALTR to them.

### LCNY token `LUSDToken.sol` and ALTR token `LQTYToken.sol`
...
Troves in Alternity are recorded in a sorted doubly linked list, sorted by their NICR, from high to low. NICR stands for the nominal collateral ratio that is simply the amount of collateral (in ETH) multiplied by 100e18 and divided by the amount of debt (in LCNY), without taking the ETH:CNY price into account. Given that all Troves are equally affected by Ether price changes, they do not need to be sorted by their real ICR.
...
`redeemCollateral` will only redeem from Troves that have an ICR >= MCR. In other words, if there are Troves at the bottom of the SortedTroves list that are below the minimum collateralization ratio (which can happen after an ETH:CNY price drop), they will be skipped. To make this more gas-efficient, the position of the first redeemable Trove should be passed as `_firstRedemptionHint`.
...
It’s likely that the last Trove in the redemption sequence would be partially redeemed from - i.e. only some of its debt cancelled with LCNY. In this case, it should be reinserted somewhere between top and bottom of the list. The `_lowerPartialRedemptionHint` and `_upperPartialRedemptionHint` hints passed to `redeemCollateral` describe the future neighbors the expected reinsert position.
...
If not, the redemption sequence doesn’t perform the final partial redemption, and terminates early. This ensures that the transaction doesn’t revert, and most of the requested LCNY redemption can be fulfilled.

## Gas compensation

In Alternity, we want to maximize liquidation throughput, and ensure that undercollateralized Troves are liquidated promptly by “liquidators” - agents who may also hold Stability Pool deposits, and who expect to profit from liquidations.
...
Gas compensation is paid in a mix of LCNY and ETH. While the ETH is taken from the liquidated Trove, the LCNY is provided by the borrower. When a borrower first issues debt, some LCNY is reserved as a Liquidation Reserve. A liquidation transaction thus draws ETH from the trove(s) it liquidates, and sends the both the reserved LCNY and the compensation in ETH to the caller, and liquidates the remainder.

When a liquidation transaction liquidates multiple Troves, each Trove contributes LCNY and ETH towards the total compensation for the transaction.

Gas compensation per liquidated Trove is given by the formula:

Gas compensation = `2000 LCNY + 0.5% of trove’s collateral (ETH)`
...
When a borrower opens a Trove, an additional 2000 LCNY debt is issued, and 2000 LCNY is minted and sent to a dedicated contract (`GasPool`) for gas compensation - the "gas pool".

When a borrower closes their active Trove, this gas compensation is refunded: 2000 LCNY is burned from the gas pool's balance, and the corresponding 2000 LCNY debt on the Trove is cancelled.

The purpose of the 2000 LCNY Liquidation Reserve is to provide a minimum level of gas compensation, regardless of the Trove's collateral size or the current ETH price.

### Liquidation

When a Trove is liquidated, 0.5% of its collateral is sent to the liquidator, along with the 2000 LCNY Liquidation Reserve. Thus, a liquidator always receives `{2000 LCNY + 0.5% collateral}` per Trove that they liquidate. The collateral remainder of the Trove is then either offset, redistributed or a combination of both, depending on the amount of LCNY in the Stability Pool.

### Gas compensation and redemptions

When a Trove is redeemed from, the redemption is made only against (debt - 2000), not the entire debt.

But if the redemption causes an amount (debt - 2000) to be cancelled, the Trove is then closed: the 2000 LCNY Liquidation Reserve is cancelled with its remaining 2000 debt. That is, the gas compensation is burned from the gas pool, and the 2000 debt is zero’d. The ETH collateral surplus from the Trove remains in the system, to be later claimed by its owner.
...
Any LCNY holder may deposit LCNY to the Stability Pool. It is designed to absorb debt from liquidations, and reward depositors with the liquidated collateral, shared between depositors in proportion to their deposit size.

Since liquidations are expected to occur at an ICR of just below 110%, and even in most extreme cases, still above 100%, a depositor can expect to receive a net gain from most liquidations. When that holds, the dollar value of the ETH gain from a liquidation exceeds the dollar value of the LCNY loss (assuming the price of LCNY is ¥1).  

We define the **collateral surplus** in a liquidation as `¥(ETH) - debt`, where `¥(...)` represents the dollar value.

At an LCNY price of ¥1, Troves with `ICR > 100%` have a positive collateral surplus.

After one or more liquidations, a deposit will have absorbed LCNY losses, and received ETH gains. The remaining reduced deposit is the **compounded deposit**.
...
When a liquidation hits the Stability Pool, it is known as an **offset**: the debt of the Trove is offset against the LCNY in the Pool. When **x** LCNY debt is offset, the debt is cancelled, and **x** LCNY in the Pool is burned. When the LCNY Stability Pool is greater than the debt of the Trove, all the Trove's debt is cancelled, and all its ETH is shared between depositors. This is a **pure offset**.

It can happen that the LCNY in the Stability Pool is less than the debt of a Trove. In this case, the the whole Stability Pool will be used to offset a fraction of the Trove’s debt, and an equal fraction of the Trove’s ETH collateral will be assigned to Stability Providers. The remainder of the Trove’s debt and ETH gets redistributed to active Troves. This is a **mixed offset and redistribution**.
...
Deposit functionality is handled by `StabilityPool.sol` (`provideToSP`, `withdrawFromSP`, etc).  StabilityPool also handles the liquidation calculation, and holds the LCNY and ETH balances.

When a liquidation is offset with the Stability Pool, debt from the liquidation is cancelled with an equal amount of LCNY in the pool, which is burned. 
...
Here’s an example of the Stability Pool absorbing liquidations. The Stability Pool contains 3 depositors, A, B and C, and the ETH:CNY price is 100.

There are two Troves to be liquidated, T1 and T2:

|   | Trove | Collateral (ETH) | Debt (LCNY) | ICR         | ¥(ETH) (¥) | Collateral surplus (¥) |
|---|-------|------------------|-------------|-------------|------------|------------------------|
...
Now, the first liquidation T1 is absorbed by the Pool: 150 debt is cancelled with 150 Pool LCNY, and its 1.6 ETH is split between depositors. We see the gains earned by A, B, C, are in proportion to their share of the total LCNY in the Stability Pool:
...
And now the second liquidation, T2, occurs: 225 debt is cancelled with 225 Pool LCNY, and 2.45 ETH is split between depositors. The accumulated ETH gain includes all ETH gain from T1 and T2.
...
Depositors deposit LCNY via `provideToSP`, and withdraw with `withdrawFromSP`. Their accumulated ETH gain is paid out every time they make a deposit operation - so ETH payout is triggered by both deposit withdrawals and top-ups.
...
## ALTR Issuance to Stability Providers

Stability Providers earn ALTR tokens continuously over time, in proportion to the size of their deposit. This is known as “Community Issuance”, and is handled by `CommunityIssuance.sol`.

Upon system deployment and activation, `CommunityIssuance` holds an initial ALTR supply, currently (provisionally) set at 32 million ALTR tokens.
...
When a deposit earns ALTR, it is split between the depositor, and the front end through which the deposit was made. Upon registering as a front end, a front end chooses a “kickback rate”: this is the percentage of ALTR earned by a tagged deposit, to allocate to the depositor. Thus, the total ALTR received by a depositor is the total ALTR earned by their deposit, multiplied by `kickbackRate`. The front end takes a cut of `1-kickbackRate` of the ALTR earned by the deposit.

### ALTR Issuance schedule

The overall community issuance schedule for ALTR is sub-linear and monotonic. We currently (provisionally) implement a yearly “halving” schedule, described by the cumulative issuance function:

`supplyCap * (1 - 0.5^t)`

where `t` is year and `supplyCap` is (provisionally) set to represent 50 million ALTR tokens.

It results in the following cumulative issuance schedule for the community ALTR supply:

| Year | Total community ALTR issued |
|------|-----------------------------|
| 0    | 0%                          |
| 1    | 50%                         |
| 2    | 75%                         |
...
The shape of the ALTR issuance curve is intended to incentivize both early depositors, and long-term deposits.

Although the ALTR issuance curve follows a yearly halving schedule, in practice the `CommunityIssuance` contract use time intervals of one minute, for more fine-grained reward calculations.

### ALTR Issuance implementation

The continuous time-based ALTR issuance is chunked into discrete reward events, that occur at every deposit change (new deposit, top-up, withdrawal), and every liquidation, before other state changes are made.

In a ALTR reward event, the ALTR to be issued is calculated based on time passed since the last reward event, `block.timestamp - lastLQTYIssuanceTime`, and the cumulative issuance function.

The ALTR produced in this issuance event is shared between depositors, in proportion to their deposit sizes.

To efficiently and accurately track ALTR gains for depositors and front ends as deposits decrease over time from liquidations, we re-use the [algorithm for rewards from a compounding, decreasing stake](https://github.com/liquity/dev/blob/main/packages/contracts/mathProofs/Scalable%20Compounding%20Stability%20Pool%20Deposits.pdf). It is the same algorithm used for the ETH gain from liquidations.

The same product `P` is used, and a sum `G` is used to track ALTR rewards, and each deposit gets a new snapshot of `P` and `G` when it is updated.

### Handling the front end ALTR gain

As mentioned in [ALTR Issuance to Stability Providers](#altr-issuance-to-stability-providers), in a ALTR reward event generating `LQTY_d` for a deposit `d` made through a front end with kickback rate `k`, the front end receives `(1-k) * LQTY_d` and the depositor receives `k * LQTY_d`.

The front end should earn a cut of ALTR gains for all deposits tagged with its front end.

Thus, we use a virtual stake for the front end, equal to the sum of all its tagged deposits. The front end’s accumulated ALTR gain is calculated in the same way as an individual deposit, using the product `P` and sum `G`.
...
### ALTR reward events and payouts

When a deposit is changed (top-up, withdrawal):

- A ALTR reward event occurs, and `G` is updated
- Its ETH and ALTR gains are paid out
- Its tagged front end’s ALTR gains are paid out to that front end
...
## Alternity System Fees

Alternity generates fee revenue from certain operations. Fees are captured by the ALTR token.

A ALTR holder may stake their ALTR, and earn a share of all system fees, proportional to their share of the total ALTR staked.

Alternity generates revenue in two ways: redemptions, and issuance of new LCNY tokens.

Redemptions fees are paid in ETH. Issuance fees (when a user opens a Trove, or issues more LCNY from their existing Trove) are paid in LCNY.
...
The issuance fee is charged on the LCNY drawn by the user and is added to the Trove's LCNY debt. It is based on the current borrowing rate.

When new LCNY are drawn via one of the `BorrowerOperations` functions `openTrove`, `withdrawLUSD` or `adjustTrove`, an extra amount `LUSDFee` is minted, and an equal amount of debt is added to the user’s Trove. The `LUSDFee` is transferred to the staking contract, `LQTYStaking.sol`.

### Fee Schedule

Redemption and issuance fees are based on the `baseRate` state variable in TroveManager, which is dynamically updated. The `baseRate` increases with each redemption, and decays according to time passed since the last fee event - i.e. the last redemption or issuance of LCNY.
...
### Staking ALTR and earning fees

ALTR holders may `stake` and `unstake` their ALTR in the `LQTYStaking.sol` contract. 

When a fee event occurs, the fee in LCNY or ETH is sent to the staking contract, and a reward-per-unit-staked sum (`F_ETH`, or `F_LUSD`) is incremented. A ALTR stake earns a share of the fee equal to its share of the total ALTR staked, at the instant the fee occurred.
...
The Alternity implementation relies on some important system properties and mathematical derivations.
...
_**LCNY**_:  The stablecoin that may be issued from a user's collateralized debt position and freely transferred/traded to any Ethereum address. Intended to maintain parity with the US dollar, and can always be redeemed directly with the system: 1 LCNY is always exchangeable for ¥1 CNY worth of ETH.
...
_**Active debt:**_ the amount of LCNY debt recorded on a Trove’s struct
...
_**Individual collateralization ratio (ICR):**_ a Trove's ICR is the ratio of the dollar value of its entire collateral at the current ETH:CNY price, to its entire debt
...
_**Total active debt:**_ the sum of active debt over all Troves. Equal to the LCNY in the ActivePool.

_**Total defaulted collateral:**_ the total ETH collateral in the DefaultPool

_**Total defaulted debt:**_ the total LCNY debt in the DefaultPool
...
_**Total collateralization ratio (TCR):**_ the ratio of the dollar value of the entire system collateral at the current ETH:CNY price, to the entire system debt

_**Critical collateralization ratio (CCR):**_ 150%. When the TCR is below the CCR, the system enters Recovery Mode.

_**Borrower:**_ an externally owned account or contract that locks collateral in a Trove and issues LCNY tokens to their own address. They “borrow” LCNY tokens against their ETH collateral.

_**Depositor:**_ an externally owned account or contract that has assigned LCNY tokens to the Stability Pool, in order to earn returns from liquidations, and receive ALTR token issuance.

_**Redemption:**_ the act of swapping LCNY tokens with the system, in return for an equivalent value of ETH. Any account with a LCNY token balance may redeem them, whether or not they are a borrower.

When LCNY is redeemed for ETH, the ETH is always withdrawn from the lowest collateral Troves, in ascending order of their collateralization ratio. A redeemer can not selectively target Troves with which to swap LCNY for ETH.

_**Repayment:**_ when a borrower sends LCNY tokens to their own Trove, reducing their debt, and increasing their collateralization ratio.
...
_**Collateral Surplus**_: The difference between the dollar value of a Trove's ETH collateral, and the dollar value of its LCNY debt. In a full liquidation, this is the net gain earned by the recipients of the liquidation.

_**Offset:**_ cancellation of liquidated debt with LCNY in the Stability Pool, and assignment of liquidated collateral to Stability Pool depositors, in proportion to their deposit.
...
_**Pure offset:**_  when a Trove's debt is entirely cancelled with LCNY in the Stability Pool, and all of it's liquidated ETH collateral is assigned to Stability Providers.

_**Mixed offset and redistribution:**_  When the Stability Pool LCNY only covers a fraction of the liquidated Trove's debt.  This fraction of debt is cancelled with LCNY in the Stability Pool, and an equal fraction of the Trove's collateral is assigned to depositors. The remaining collateral & debt is redistributed directly to active Troves.

_**Gas compensation:**_ A refund, in LCNY and ETH, automatically paid to the caller of a liquidation function, intended to at least cover the gas cost of the transaction. Designed to ensure that liquidators are not dissuaded by potentially high gas costs.

## Development

The Alternity monorepo is based on Yarn's [workspaces](https://classic.yarnpkg.com/en/docs/workspaces/) feature. You might be able to install some of the packages individually with npm, but to make all interdependent packages see each other, you'll need to use Yarn.
...
- [Node v14.x](https://nodejs.org/dist/latest-v14.x/)
- [Docker](https://docs.docker.com/get-docker/)
- [Yarn](https://classic.yarnpkg.com/en/docs/install)

#### Making node-gyp work

Alternity indirectly depends on some packages with native addons. To make sure these can be built, you'll have to take some additional steps. Refer to the subsection of [Installation](https://github.com/nodejs/node-gyp#installation) in node-gyp's README that corresponds to your operating system.

Note: you can skip the manual installation of node-gyp itself (`npm install -g node-gyp`), but you will need to install its prerequisites to make sure Alternity can be installed.

### Clone & Install

```
git clone https://github.com/0xalternity/dev.git alternity
cd alternity
yarn
```
...
Stability Pool depositors expect to make profits from liquidations which are likely to happen at a collateral ratio slightly below 110%, but well above 100%. In rare cases (flash crashes, oracle failures), troves may be liquidated below 100% though, resulting in a net loss for stability depositors. Depositors thus have an incentive to withdraw their deposits if they anticipate liquidations below 100% (note that the exact threshold of such “unprofitable” liquidations will depend on the current CNY price of LCNY).

As long the difference between two price feed updates is <10% and price stability is maintained, loss evasion situations should be rare. The percentage changes between two consecutive prices reported by Chainlink’s ETH:USD and CNY:USD oracle has only ever come close to 10% a handful of times in the past few years.
...
This solution is only partially effective since it does not prevent stability depositors from monitoring price feeds and front-running oracle price update transactions that would make troves liquidatable. Given that we expect loss-evasion opportunities to be very rare, we do not expect that a significant fraction of stability depositors would actually apply front-running strategies, which require sophistication and automation. In the unlikely event that large fraction of the depositors withdraw shortly before the liquidation of troves at <100% CR, the redistribution mechanism will still be able to absorb defaults.
...
Front-runners could deposit funds to the Stability Pool on the fly (instead of keeping their funds in the pool) and make liquidation gains when they see a pending price update or liquidate transaction. They could even borrow the LCNY using a trove as a flash loan.
